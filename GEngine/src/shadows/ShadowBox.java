...
public class ShadowBox {
	...
	public static Vector3f toVec3f(GVector3f vec){
...
//		toNear.scale(MasterRenderer.NEAR_PLANE);
		toNear.scale(cam.NEAR_PLANE);
		
		
		Vector3f centerNear = Vector3f.add(toNear, toVec3f(cam.getPosition()), null);
		Vector3f centerFar = Vector3f.add(toFar, toVec3f(cam.getPosition()), null);
	...
	private void calculateWidthsAndHeights() {
		farWidth = (float) (SHADOW_DISTANCE * Math.tan(Math.toRadians(cam.FOV)));
		
		nearWidth = (float) (cam.NEAR_PLANE
				* Math.tan(Math.toRadians(cam.FOV)));
